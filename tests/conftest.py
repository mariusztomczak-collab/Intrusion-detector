...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if any(keyword in item.name.lower() for keyword in ["security", "auth", "threat"]):
            item.add_marker(pytest.mark.security)
        
        # Add ml marker to ML-related tests
        if any(keyword in item.name.lower() for keyword in ["ml", "model", "preprocessor", "classification"]):
        ...
        if any(keyword in item.name.lower() for keyword in ["ui", "gradio", "interface"]):
