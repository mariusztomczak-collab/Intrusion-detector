...
def load_data(data_path: str) -> Tuple[pd.DataFrame, pd.Series]:
    ...
    try:
        ...
        logger.info(
            f"Loaded data with {len(X)} samples and {len(X.columns)} selected features"
        )
...
def main():
    ...
    parser.add_argument(
        "--test_data_path", type=str, required=True, help="Path to the test dataset"
    )
    parser.add_argument(
        "--n_features", type=int, default=10, help="Number of features to select"
    )
    parser.add_argument(
