...
class ColumnTransformerWrapper:
    ...
    def __init__(self, column_transformer: ColumnTransformer):
        ...
        logger.info(
            f"Initialized ColumnTransformerWrapper with features: {self.selected_features}"
        )
                ...
                if name == "num":
                    ...
                    if hasattr(transformer, "categories_"):
                        for i, feature in enumerate(features):
                            ...
                            if (
                                hasattr(transformer, "drop")
                                and transformer.drop == "first"
                            ):
                                categories = categories[1:]
                            feature_names.extend(
                                [f"{feature}_{val}" for val in categories]
                            )
                    else:
...
class DataPreprocessor:
    def __init__(self):
        ...
        self.numerical_features = [
            f for f in self.selected_features if f not in self.categorical_features
        ]

        logger.info(
            f"Initialized preprocessor with {len(self.numerical_features)} numerical and "
    ...
    def transform(self, X: pd.DataFrame) -> np.ndarray:
        ...
        if "flag_SF" in self.get_feature_names():
            ...
            X_transformed = np.hstack(
                [
                    X_transformed[:, : len(self.numerical_features)],
                    ...
                    for i, feature in enumerate(features):
                        ...
                        if feature == "flag":
                            ...
                            encoded_features.extend(
                                [f"{feature}_{val}" for val in encoder.categories_[i]]
                            )
