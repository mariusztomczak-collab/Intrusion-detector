...
class ModelTrainer:
    ...
    def train_and_evaluate(
        ...
        if len(y_train) != X_train.shape[0]:
            raise ValueError(f"Train data length mismatch: X {X_train.shape[0]} != y {len(y_train)}")
        if len(y_test) != X_test.shape[0]:
            raise ValueError(f"Test data length mismatch: X {X_test.shape[0]} != y {len(y_test)}")
                ...
                with mlflow.start_run(run_name=model_name):
                    ...
                    if metrics['f1'] > self.best_score:
                        ...
                        try:
                            ...
                            logger.info(f"Registered best model ({model_name}) in MLflow Model Registry")
                        except Exception as e:
                            logger.error(f"Failed to register model in MLflow Model Registry: {str(e)}")
