...
async def verify_credentials(credentials: HTTPAuthorizationCredentials = Depends(security)) -> str:
    """
    ...
    try:
        ...
        logger.info(f"DEBUG: Auth verification successful for user: {user_data.get('email')}")
        return user_id
