...
class AuthLoginTab:
    def __init__(self):
        self.email = gr.Textbox(
            label="📧 Adres email", placeholder="twoj.email@example.com", type="email"
        )
        self.password = gr.Textbox(
            label="🔒 Hasło", placeholder="Wprowadź swoje hasło", type="password"
        )
    ...
    def handle_login(self, email, password):
        ...
        try:
            # Debug logging
            logger.info(
                f"Login attempt - email type: {type(email)}, password type: {type(password)}"
            )
