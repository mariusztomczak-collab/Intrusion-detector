...
class UnifiedApp:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    if "Invalid email or password" in error_msg or "Invalid credentials" in error_msg:
                        error_msg = "❌ Nieprawidłowy email lub hasło. Sprawdź swoje dane i spróbuj ponownie."
                    elif "User not found" in error_msg:
                        error_msg = "❌ Użytkownik o podanym adresie email nie istnieje."
                    else:
    ...
    def handle_register(self, email, password, confirm_password):
        ...
        try:
            ...
            if not self.validate_email(email):
                return "❌ Nieprawidłowy format adresu email.", "", gr.update(selected=1)
            ...
            return f"❌ Wystąpił błąd podczas rejestracji: {str(e)}", "", gr.update(selected=1)
    ...
    def classify_traffic(self, logged_in, count, serror_rate, srv_serror_rate, same_srv_rate,
                        dst_host_srv_count, dst_host_same_srv_rate, dst_host_serror_rate,
                        dst_host_srv_serror_rate, flag, auth_state):
        ...
        try:
            ...
            response = requests.post(f"{self.api_base_url}/decisions/single", json=traffic_data, headers=headers)
    ...
    def create_app(self):
        ...
        with gr.Blocks(
            ...
            gr.Markdown("### System analizy ruchu sieciowego z autentykacją użytkowników")
                ...
                with gr.Tab("🔐 Logowanie", id=0):
                    with gr.Column(elem_classes=["auth-container"]):
                        with gr.Column(elem_classes=["login-form"]):
                            ...
                            gr.Markdown("*Wymagane: Użytkownik NIE MOŻE korzystać z funkcji aplikacji bez logowania (US-007)*")
                            ...
                            login_error = gr.Textbox(label="", visible=True, interactive=False, elem_classes=["login-error"])
                            login_status = gr.Textbox(label="", visible=True, interactive=False, elem_classes=["login-status"])
                            login_button = gr.Button("🔐 Zaloguj się", variant="primary")
                            
                            # Link to registration
                            gr.Markdown("---")
                            gr.Markdown("**Nie masz konta?** [Przejdź do zakładki Rejestracja](#)")
                ...
                with gr.Tab("📝 Rejestracja", id=1):
                    with gr.Column(elem_classes=["auth-container"]):
                        with gr.Column(elem_classes=["register-form"]):
                            ...
                            gr.Markdown("*Hasło musi mieć minimum 8 znaków zgodnie z wymaganiami Supabase*")
                            ...
                            register_error = gr.Textbox(label="", visible=True, interactive=False, elem_classes=["register-error"])
                            register_status = gr.Textbox(label="", visible=True, interactive=False, elem_classes=["register-status"])
                            register_button = gr.Button("📝 Zarejestruj się", variant="primary")
                            
                            # Link to login
                            gr.Markdown("---")
                            gr.Markdown("**Masz już konto?** [Przejdź do zakładki Logowanie](#)")
                
                # Analysis tab (protected) - index 2
                with gr.Tab("🔍 Analiza ruchu", id=2):
                    # Logout section
                    with gr.Row(elem_classes=["logout-container"]):
                        user_info = gr.Markdown("👤 Zalogowany jako: Nie zalogowany", elem_classes=["user-info"])
                        logout_button = gr.Button("🚪 Wyloguj się", variant="secondary", elem_classes=["logout-button"])
                    
                    # Protected content - only visible after authentication
                    with gr.Column(elem_classes=["analysis-container", "protected-content"]):
                        gr.Markdown("## 🔍 Analiza ruchu sieciowego")
                        gr.Markdown("**Wprowadź parametry ruchu sieciowego, aby wykryć potencjalne zagrożenia.**")
                        ...
                        with gr.Row():
                            with gr.Column(scale=1):
                                ...
                                count = gr.Number(label="Liczba połączeń", value=45, minimum=0)
                                serror_rate = gr.Slider(label="Współczynnik błędów SYN", value=0.05, minimum=0, maximum=1, step=0.01)
                                srv_serror_rate = gr.Slider(label="Współczynnik błędów SYN serwisu", value=0.04, minimum=0, maximum=1, step=0.01)
                                same_srv_rate = gr.Slider(label="Współczynnik tego samego serwisu", value=0.88, minimum=0, maximum=1, step=0.01)
                                
                            with gr.Column(scale=1):
                                gr.Markdown("### 🌐 Parametry hosta docelowego")
                                
                                dst_host_srv_count = gr.Number(label="Liczba serwisów hosta docelowego", value=110, minimum=0)
                                dst_host_same_srv_rate = gr.Slider(label="Współczynnik tego samego serwisu hosta docelowego", value=0.99, minimum=0, maximum=1, step=0.01)
                                dst_host_serror_rate = gr.Slider(label="Współczynnik błędów SYN hosta docelowego", value=0.02, minimum=0, maximum=1, step=0.01)
                                dst_host_srv_serror_rate = gr.Slider(label="Współczynnik błędów SYN serwisu hosta docelowego", value=0.01, minimum=0, maximum=1, step=0.01)
                                flag = gr.Dropdown(label="Flaga", choices=["S0", "SF"], value="S0")
                        
                        with gr.Row():
                            analyze_btn = gr.Button("🔍 Analizuj ruch", variant="primary", size="lg")
                        
                        with gr.Row():
                            output = gr.Textbox(label="Wynik analizy", lines=5, interactive=False)
                        ...
                        with gr.Row():
                            normal_btn = gr.Button("Wczytaj normalny ruch", variant="secondary")
                            malicious_btn = gr.Button("Wczytaj złośliwy ruch", variant="secondary")
            ...
            logout_button.click(
                ...
                inputs=[auth_state],
                outputs=[auth_state, tabs]
            ).then(
                fn=lambda: "👤 Zalogowany jako: Nie zalogowany",
                outputs=user_info
            )
            ...
            analyze_btn.click(
                ...
                inputs=[logged_in, count, serror_rate, srv_serror_rate, same_srv_rate,
                       dst_host_srv_count, dst_host_same_srv_rate, dst_host_serror_rate,
                       dst_host_srv_serror_rate, flag, auth_state],
                outputs=output
            )
            ...
            normal_btn.click(
                ...
                outputs=[logged_in, count, serror_rate, srv_serror_rate, same_srv_rate,
                        dst_host_srv_count, dst_host_same_srv_rate, dst_host_serror_rate,
